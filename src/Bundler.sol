...
contract Bundler {
    ...
    function _prepareMulticurveQuote(
        CreateParams calldata createData
    ) private returns (address asset, PoolKey memory poolKey, bool zeroForOne) {
        (asset, , , , ) = airlock.create(createData);
        (, , poolKey, ) = UniswapV4MulticurveInitializer(
    ...
    function _resolveExactOutAmount(
        ...
        if (overrideAmount != 0) {
            ...
            if (numTokensToSell > type(uint128).max)
                revert ExactAmountTooLarge();
