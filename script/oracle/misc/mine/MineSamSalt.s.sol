...
/**
 * @title MineSamSalt
 * @notice Mine for a CREATE2 salt that produces a SAM token address with:
 *         1. Last byte == 0x69
...
contract MineSamSalt is Script {
    ...
    // Token configuration (must match LaunchSam.s.sol)
    string constant NAME = "SAM";
    string constant SYMBOL = "SAM";
    string constant TOKEN_URI =
        '{"name":"SAM","symbol":"SAM","image":"ipfs://QmNasq6irYVCs59tTPNgXuyQoMS9JZVR1tyjCayL51wLSv","x":"https://x.com/samterminalcom"}';
    uint256 constant INITIAL_SUPPLY = 100_000_000_000 ether;

    function run() public returns (bytes32, address) {
        console2.log("=== Mine SAM Salt ===");
        ...
        for (uint256 i = 0; i < startPatterns.length; i++) {
            ...
            if (endsIn69 && lessThanNoice) {
                ...
                console2.log(
                    "   forge script script/oracle/launch/LaunchSam.s.sol \\"
                );
