...
contract LaunchOracle is Script {
    // NoiceLaunchpad address
    address constant LAUNCHPAD = 0xF29C64592738f94b63380b81BfD0488DebE9A699;
    address constant AIRLOCK = 0x660eAaEdEBc968f8f3694354FA8EC0b4c5Ba8D12;
    ...
    // Token amounts (Total: 100B)
    // NOICE price: $0.0003918
    uint256 constant CREATOR_VESTING = 30_000_000_000 ether; // 30B (30%)
    uint256 constant CREATOR_UNLOCKED = 15_000_000_000 ether; // 5B (5%)

    uint256 constant SSL_TRANCHE_1 = 2_000_000_000 ether; // 2B - $1M-$3M
    uint256 constant SSL_TRANCHE_2 = 3_000_000_000 ether; // 3B - $3M-$6M
    // uint256 constant SSL_TRANCHE_3 = 5_000_000_000 ether; // 5B - $6M-$12M
    // uint256 constant SSL_TRANCHE_4 = 5_000_000_000 ether; // 5B - $12M-$20M
    ...
    // uint256 constant PREBUY_ALLOCATION = 10_000_000_000 ether; // 10B (10% - atomic prebuy)
    // uint256 constant PREBUY_ALLOCATION = 0 ether; // 10B (10% - atomic prebuy)
    uint256 constant PREBUY_ALLOCATION = 10_000_000_000 ether; // 10B (10% - atomic prebuy)

    uint256 constant NUM_TOKENS_TO_SELL = 50_000_000_000 ether; // 50B (10B prebuy liq + 5B early + 35B late)
    uint256 constant MAX_NOICE_INPUT = 500 ether; // 200M NOICE max for prebuy

    // Tick boundaries (calculated with NOICE = $0.0003918, 40B tokens in circulation)
    // int24 constant TICK_200K = -52800; // $200K (prebuy liquidity start)
    // int24 constant TICK_250K = -50520; // $250K (multicurve start)
    // int24 constant TICK_1M = -36660; // $1M
    // int24 constant TICK_3M = -25680; // $3M
    // int24 constant TICK_6M = -18780; // $6M
    // int24 constant TICK_12M = -11820; // $12M
    // int24 constant TICK_20M = -6720; // $20M (public late start)
    // int24 constant TICK_1_5B = 36480; // $1.5B (multicurve end)

    int24 constant TICK_0 = -289980;
    int24 constant TICK_1 = -167940;
    int24 constant TICK_2 = -151800;
    // int24 constant TICK_1 =
    ...
    // SSL tick boundaries (must be above initial tick for token0)
    // int24 constant SSL1_TICK_LOWER = -50460; // ~$252K (slightly above $250K start)
    // int24 constant SSL1_TICK_UPPER = -36660; // $1M
    // int24 constant SSL2_TICK_LOWER = -25680; // $3M
    // int24 constant SSL2_TICK_UPPER = -18780; // $6M
    // int24 constant SSL3_TICK_LOWER = -11820; // $12M
    // int24 constant SSL3_TICK_UPPER = -6720; // $20M
    // // int24 constant SSL4_TICK_LOWER = -8220; // $10M
    // // int24 constant SSL4_TICK_UPPER = -4140; // $15M

    // int24 constant SSL1_TICK_LOWER = -36660; // $1M
    // int24 constant SSL1_TICK_UPPER = -25680; // $3M

    // int24 constant SSL2_TICK_LOWER = -25680; // $3M
    // int24 constant SSL2_TICK_UPPER = -18780; // $6M

    // int24 constant SSL3_TICK_LOWER = -18780; // $6M
    // int24 constant SSL3_TICK_UPPER = -11820; // $12M

    // int24 constant SSL4_TICK_LOWER = -11820; // $12M
    // int24 constant SSL4_TICK_UPPER = -6720; // $20M

    int24 constant SSL1_TICK_LOWER = -167940; // $1M
    int24 constant SSL1_TICK_UPPER = -165660; // $3M

    int24 constant SSL2_TICK_LOWER = -158760; // $3M
    int24 constant SSL2_TICK_UPPER = -155400; // $6M
    ...
    function run() public returns (bytes32, address) {
        uint256 privateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(privateKey);
        address feeBeneficiary = 0xb2e7Eb3fCa9510e4AB231Ad7A7a99e70a45029d9;
        address teamsAddress = 0x00946a5244F9D5104bbb4dd2f6A23b9059071bf4;
        ...
        curves[0] = Curve({
            ...
            tickUpper: TICK_1,
            numPositions: 1,
            shares: 200000000000000000 // 20% (20B tokens)
        });
        curves[1] = Curve({
            tickLower: TICK_1,
            tickUpper: TICK_2,
            numPositions: 20,
            shares: 800000000000000000 // 80% (80B tokens)
        });
        // curves[0] = Curve({
        //     tickLower: TICK_250K,
        //     tickUpper: TICK_1M,
        //     numPositions: 20,
        //     shares: 100000000000000000 // 30% (5B tokens)
        // });
        // curves[1] = Curve({
        //     tickLower: TICK_1M,
        //     tickUpper: TICK_1_5B,
        //     numPositions: 1,
        //     shares: 900000000000000000 // 70% (35B tokens)
        // });
        ...
        beneficiaries[1] = BeneficiaryData({
            beneficiary: feeBeneficiary, // Set to splits -> 80% to creator (steams splits) 20% to us
            shares: 950000000000000000 // 95%
        });
        ...
        creatorAllocations[1] = NoiceCreatorAllocation({
            ...
            amount: CREATOR_UNLOCKED,
            lockStartTimestamp: uint40(block.timestamp),
            lockEndTimestamp: uint40(block.timestamp + 1)
        });

        // LP unlock tranches
        NoiceLpUnlockTranche[]
            memory lpUnlockTranches = new NoiceLpUnlockTranche[](2);
        lpUnlockTranches[0] = NoiceLpUnlockTranche({
            amount: SSL_TRANCHE_1,
            tickLower: SSL1_TICK_LOWER,
            tickUpper: SSL1_TICK_UPPER,
        ...
        NoicePrebuyParticipant[]
            memory prebuyParticipants = new NoicePrebuyParticipant[](0);
        ...
        console.log("Configuration:");
        console.log("- NOICE Price: $0.0003918");
        console.log("- Fee: 2%");
        console.log(
            "- Prebuy Liquidity: 10B (1 position, $200K-$250K, 365d vesting)"
        );
        console.log("- Public Early: 5B (20 positions, $250K-$1M)");
        console.log("- Public Late: 35B (single position, $1M-$1.5B)");
        console.log(
            "- Prebuy: 10B (exact output, max 200M NOICE, 1yr vesting)"
        );
        console.log("- SSL1: 2B @ $1M-$3M");
        console.log("- SSL2: 3B @ $3M-$6M");
        console.log("- SSL3: 5B @ $6M-$12M");
        console.log("- SSL4: 5B @ $12M-$20M");
        console.log("- Total SSL: 15B");
        console.log("- Creator: 35B (30B vested 24mo, 5B vested 1 second)");
        console.log("- Total multicurve: 50B (10B + 5B + 35B)");
        console.log("");
