...
import {UniswapV4Migrator, UnorderedBeneficiaries, InvalidShares, InvalidTotalShares, InvalidLength, InvalidProtocolOwnerShares, InvalidProtocolOwnerBeneficiary} from "src/UniswapV4Migrator.sol";
...
contract UniswapV4MigratorTest is PosmTestSetup {
    ...
    function test_initialize_StoresPoolKey() public {
        ...
        migrator.initialize(
            address(asset),
            address(numeraire),
            abi.encode(FEE, TICK_SPACING, LOCK_DURATION, beneficiaries)
        );

        (PoolKey memory poolKey, uint256 lockDuration) = migrator.getAssetData(
            token0,
            token1
        );
    ...
    function test_migrate(
        ...
        migrator.initialize(
            asset,
            numeraire,
            abi.encode(FEE, 1, LOCK_DURATION, beneficiaries)
        );
        ...
        if (recipient != address(0xdead)) {
            assertGe(
                ERC721(address(lpm)).balanceOf(address(recipient)),
                1,
                "Wrong recipient balance"
            );
            assertGe(
                ERC721(address(lpm)).balanceOf(address(locker)),
                1,
                "Wrong locker balance with recipient"
            );
        } else {
            assertGe(
                ERC721(address(lpm)).balanceOf(address(locker)),
                1,
                "Wrong locker balance without recipient"
            );
        }

        if (isUsingETH) {
            assertEq(
                address(migrator).balance,
                0,
                "Migrator should have no ETH left"
            );
        } else {
            assertEq(
                TestERC20(token0).balanceOf(address(migrator)),
                0,
                "Migrator should have no token0 left"
            );
        }
        assertEq(
            TestERC20(token1).balanceOf(address(migrator)),
            0,
            "Migrator should have no token1 left"
        );
    }
    ...
    function test_initialize_RevertZeroAddressBeneficiary() public {
        ...
        vm.prank(address(airlock));
        vm.expectRevert(
            abi.encodeWithSelector(UnorderedBeneficiaries.selector)
        );
        migrator.initialize(
            address(asset),
            address(numeraire),
    ...
    function test_initialize_RevertInvalidProtocolOwnerShares() public {
        ...
        vm.prank(address(airlock));
        vm.expectRevert(
            abi.encodeWithSelector(InvalidProtocolOwnerShares.selector)
        );
        migrator.initialize(
            address(asset),
            address(numeraire),
    ...
    function test_initialize_RevertProtocolOwnerNotFound() public {
        ...
        vm.prank(address(airlock));
        vm.expectRevert(
            abi.encodeWithSelector(InvalidProtocolOwnerBeneficiary.selector)
        );
        migrator.initialize(
            address(asset),
            address(numeraire),
